  ...
  return (
    <main className="container-page space-y-5 py-8">
      ...
      {typeof params.error === "string" ? (
        <div className="rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-700">{params.error}</div>
      ) : null}
      {params.imported === "1" ? (
      ...
      <nav className="flex flex-wrap gap-2">
        {statuses.map((item) => (
          <Link
            ...
            className={item === safe_status ? "btn-primary" : "btn-secondary"}
          >
            {item}
          </Link>
        ))}
      ...
      <section className="space-y-3">
        {candidates.map((candidate) => (
          <article key={candidate.id} className="card p-4">
            <div className="grid gap-4 lg:grid-cols-[1fr_auto]">
              <div>
                ...
                <div className="mt-1 text-xs text-muted">
                  {candidate.source} / {candidate.source_id ?? "-"} · 联系 {candidate.parsed_phone ?? candidate.parsed_wechat ?? "缺失"} ·
                  置信度 {candidate.parse_confidence ?? 0}
                </div>
                {candidate.parse_warnings?.length ? (
                  <div className="mt-2 text-sm text-amber-700">{candidate.parse_warnings.join("；")}</div>
                ) : null}
                {candidate.source_url ? (
                  <a href={candidate.source_url} target="_blank" rel="noreferrer" className="mt-2 inline-block text-sm underline">查看来源</a>
                ) : null}
              </div>
            ...
            {candidate.import_status === "approved" ? (
              <form action={importApprovedListingCandidate} className="mt-4 flex flex-wrap gap-2 border-t border-line pt-4">
...
function StatusButton({ candidateId, status, label }: { candidateId: string; status: string; label: string }) {
  return (
    <form action={setListingImportCandidateStatus}>
      ...
      <button className={status === "approved" ? "btn-primary" : "btn-secondary"} type="submit">{label}</button>
    </form>
  );
}
