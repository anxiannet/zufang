...
create table public.nearby_places_cache (
  id uuid primary key default gen_random_uuid(),
  listing_id uuid not null references public.listings(id) on delete cascade,
...
create index listings_search_idx on public.listings using gin (to_tsvector('simple', coalesce(title, '') || ' ' || coalesce(street_name, '') || ' ' || coalesce(nearest_mrt, '') || ' ' || coalesce(description, '')));
create index listings_status_created_idx on public.listings (status, created_at desc);
create index listings_price_idx on public.listings (rent_amount);
create index listing_facilities_filter_idx on public.listing_facilities (facility_name, availability);
...
create policy "public reads listing images bucket" on storage.objects
for select using (bucket_id = 'listing-images');

create policy "authenticated uploads listing images" on storage.objects
for insert to authenticated with check (bucket_id = 'listing-images');

create policy "authenticated updates listing images" on storage.objects
for update to authenticated using (bucket_id = 'listing-images') with check (bucket_id = 'listing-images');

create policy "authenticated deletes listing images" on storage.objects
